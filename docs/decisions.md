...
## 2026-07-20 — 試跑回饋:回報合併放寬 + 時區修正 + 工項清單收起

### 一、合併現場回報不再限「與日誌同日」(推翻 2026-05-08 的限制)

- 員工經 LINE 回報:開新日誌完全看不到現場回報。追查是 5/8 加的同日過濾
  加上「當天 0 筆時提示也跟著整區消失」的條件 bug — 主任以為回報不見了。
- 先修 bug(其他日期有 pending 時顯示提示),再依 Evelyn 裁示直接放寬:
  **全部 pending 回報都列出**,同日排前面,非本日誌日期的卡片帶琥珀色
  「M/D 的回報」徽章防誤併。原因:主任常要補登別日的回報照片,
  「改日誌日期才看得到」太繞。
- 5/8 的原始顧慮(誤併別日內容)由徽章 + 說明文案承接,不再用隱藏處理。

### 二、日期一律走台北時區(`todayLocalDate()`)

- `toISOString().slice(0,10)` 是 UTC 日期:台灣 00:00–07:59 會拿到前一天
  (client 在台灣裝置踩到,server 在 Vercel UTC 必踩)。
- 新增 `lib/daily-log.ts` 的 `todayLocalDate()`,替換新日誌預設日期、
  複製日誌日期、PDF zip 檔名三處。之後任何「今天」都必須走這個 helper。

### 三、工項 picker「瀏覽全部工項」一律預設收起

- 原本未勾選時預設展開;配電盤標單 1200+ 項,一進頁面整棵樹很難滑。
- 搜尋框常駐頂部、輸入自動展開,已勾選卡片照樣置頂。

### 四、LINE OA 聊天功能開啟(收 bug 回報)
...
## 2026-07-29 — 儀表板警示可「先不理」(migration-2.30)

- 需求:老闆儀表板「需要您出手」的每一項要能各自關掉。
- **做成「先不理 7 天」而不是永久忽略**:這兩種警示(退回未重送、案件停滯)
  代表真的有東西卡住,永久隱藏 = 把問題藏起來,時間到自動再出現。
  案件若是真的暫停施工,正解是把案件狀態改「暫停中」(本來就不列入停滯)。
- per-user(老闆忽略不影響辦公室助理);upsert 同鍵 = 重新計時;
  可一鍵「復原」全部。標題列顯示「另有 N 件先不理中」不讓人忘記。
- target_id 不設 FK(對象跨 daily_logs / cases 兩張表);對象被刪掉時
  警示本來就不會產生,殘留列無害。
- migration 未跑時查詢失敗 → 當作沒有忽略紀錄,儀表板照常顯示全部。

## 2026-07-29(續)— 補上「暫停施工」UI + 修正「尚未開工」誤導

- **paused 狀態一直存在但沒有 UI**:schema、打卡選單(〔暫停〕)、案件總覽報表
  (暫停 badge)、儀表板停滯統計都認得 paused,但**沒有任何地方能把案件設成暫停** —
  等於一個做一半的功能。上一則我建議業主「把案件改暫停中」時才發現。
  補上案件頁「暫停 / 恢復施工」按鈕(office_staff / owner)。
- 暫停 vs 結案:暫停仍可填日誌 / 打卡、隨時恢復,只是不列入「案件停滯」提醒;
  結案會擋掉填寫。等材料 / 業主喊停 / 天候停工用暫停才對。
- **「尚未開工」誤導**:標籤只看 started_at 是否有值,所以「已經填了日誌但沒填
  開工日」的案件也顯示「尚未開工」。改成有日誌時顯示「開工日未填」—
  講的是資料缺漏而不是工程狀態。

## 2026-08-01 — 業主四項回饋:助理全權改日誌、修改標記、點工、核定文案

四項來自裕民老闆的回饋,一批做完。

### 1. 辦公室助理全權修改日誌 → 補上「撤回核定」(migration-2.31)

- 起點是誤會:助理**本來就能**改「簽核中 / 已退回」的日誌全部內容
  (工項、數量、照片、備註),RLS(migration-2.10 `logs_office_write`)與
  `saveLogAction` 的 post_edit 分支都早就開好了。真正缺的是**已核定**那一段。
- **不開放直接編輯已核定日誌**:核定 = 兩位核定人已簽名 + PDF 已產出(可能已
  對外寄出)。直接改會讓「已簽名的那份 PDF」跟系統內容對不上。改走
  **撤回核定 → 回到 audit 關 → 改完重走核定**:
  本輪 approve_signatures 歸零(兩位都要重簽)、pdf_status 打回 pending
  (重新核定時 upsert 覆蓋同一路徑)、log_approvals 留一筆 `[撤回核定] 原因`。
- **migration-2.31 的由來**:migration-2.10 的 INSERT policy 是嚴格 stage×role,
  approve 關只有 owner 能寫 → 助理撤回時那筆軌跡會被 RLS 靜靜擋掉。
  新 policy 只放行「approve 關 + decision='rejected'」給 office_staff / owner,
  核定「通過」的權限沒有外流。**同一個洞也讓助理強制退回卡在核定關的日誌時
  寫不進軌跡**,一併修好。
- 軌跡先寫、狀態後改(同 post_edit):寧可留下「嘗試撤回」的紀錄,
  也不要日誌被撤回卻查不到是誰做的。撤回後舊 PDF 不再開放下載
  (`getPdfDownloadUrlAction` 加 status 檢查),避免流出跟內容對不上的版本。

### 2. 「經助理修改」標籤 + 前後對照(lib/log-diff.ts)

- 標籤掛在日誌列表、日誌詳情頁標題列(可點,跳編輯軌跡)、待簽核清單與簽核詳情頁
  — 老闆簽名前就看得到這份被辦公室動過。
- 對照資料**不需要新欄位**:`daily_log_revisions.snapshot` 從 migration-2.9 起
  就存了「編輯前的完整快照」,以前只是沒拿來用(UI 只列欄位名)。
  串法:revisions 依時間新到舊,第 N 筆的「改後」= 第 N-1 筆的 snapshot,
  最新一筆的「改後」= daily_logs 現值。
- 只做「人看得懂的摘要」不做逐字 diff:數量、人數、工項增刪、照片張數、
  文字原文。原文用小字 + 刪除線(業主:不用很顯眼但要看得到)。
- 工項名稱查表要**併入快照裡的工項 id** — 只查現值的話,「被助理刪掉的工項」
  會查不到名字。

### 3. 點工(day_labor)— 不動 schema

- manpower 是 jsonb,直接加 `day_labor` / `day_labor_accumulated` /
  `day_labor_note`,**不需要 migration**。
- **與出工人次分開累計**(`computeDayLaborByCase` 獨立一條線,不併入
  `today_total`):點工是臨時人力,只請款、不簽合約,兩者混在一起會讓
  合約人力統計失真。PDF 也另立一區「點工(臨時人力,不列入出工人次)」。
- 備註欄必要:辦公室請款要有依據(「這 2 個點工今天做什麼」)。
- 註:舊的 `unsigned_items.category` 有個 '點工' 選項,那是自由填寫的遺留欄位,
  跟這次的結構化欄位無關。

### 4. 核定雙簽文案改中性

- 業主不想在畫面上出現「兩個老闆」。全站雙簽相關文案 `老闆` → `核定人`
  (「等另一位核定人補簽」「核定要兩位核定人都簽名」),含 LINE 通知。
- **保留「還差一簽」的資訊**(業主選的是「只換稱呼」而非「完全不提兩人」):
  簽核者要知道自己簽完為什麼還沒結案。
...
## 2026-08-03(續)— 標單附加匯入 + 撤銷匯入不再洗資料

業主回報:同案件匯第二份標單會「把原本的資料洗掉」,按撤銷後日誌裡的工項
全變「(已刪除工項)」。追查是**兩個疊在一起的設計缺陷**:

### 根因

1. 匯入只有「合併」一種模式,而合併是為「重匯同一份檔」設計的:
   以 (tender_code, name) 比對,撞名的既有工項會被新檔覆蓋數量單價、
   改掛到新檔的樹底下(parent_id / sort_path 全換),**import_id 也被改成新匯入**。
   兩份不同標單撞名機率很高(分類層「假設工程」之類幾乎必撞)。
2. 撤銷匯入 = `DELETE WHERE import_id = 此次匯入`。因為 1. 的 import_id 改寫,
   原有工項也被算進「此次匯入」→ 一撤銷整批消失;另外分類層被刪時
   parent_id 的 ON DELETE CASCADE 會把「手動修改過」的子項一起帶走,
   跟確認對話框承諾的「會保留」不符。

### 修法

- **新增「附加匯入」模式,案件已有工項時為預設**:全部當新列插入、
  完全不比對,sort_path 第一段整批位移到既有樹最大 root 段之後
  (`lib/import-plan.ts` 的 `offsetSortPath`),新樹排在既有樹後面。
  匯另一份標單(追加、分包)走這條;「合併更新」保留給重匯同一份檔,
  UI 兩張 radio 卡片講清楚各自適用時機。
- **合併更新不再改既有列的 import_id** — 既有工項永遠留在自己原本的
  匯入批次,撤銷新匯入就只會刪到新匯入真正插入的列。
- **撤銷改為「安全刪除集」**(`planUndoImport`,純函式有單元測試):
  一列可刪 ⇔ 屬於此次匯入 && 未手動修改 && 未被任何日誌引用(含草稿)
  && 所有子孫都可刪。手動修改過、日誌引用中、底下還有存活子孫的分類層
  一律保留 — 這樣 CASCADE 永遠不會誤傷要保留的列。刪除分批 100 筆
  (.in() 走 URL,一次塞上千 id 會爆長度)。保留列的 import_id 由
  tender_imports 刪除時的 FK `on delete set null` 自動歸零。
- 撤銷從 redirect 改回傳結果物件,client toast 報「移除 N 筆、保留:
  日誌已引用 X 項、手動修改過 Y 項」— 使用者當下就知道為什麼有東西留著。
- 案件頁「最後匯入」擴成**匯入紀錄清單**(近 5 筆),每筆可個別撤銷 —
  多份標單並存後,「只能撤最後一筆」不夠用。
- 免 migration:全部是應用層行為,schema 不動。
- 教訓:「以 dedupe key 合併」隱含「同一份檔」假設;資料層的批次歸屬
  (import_id)一旦被改寫,依附其上的批次操作(撤銷)就會誤殺。
  批次歸屬欄位應視為不可變。

## 2026-08-04 — 標單比對鍵改成章節路徑 + 進度算法改產值加權

接續 08-03 那批修法。業主回報「退回的日誌裡工項還是沒回來、累計進度也沒回來」,
查下去發現 08-03 修掉的是「撤銷會誤刪」,但**合併匯入的比對鍵本身也是壞的**,
另外進度算法有一個一直沒被發現的歸零 bug。

### 1. dedupe key:(tender_code, name) → 章節路徑 + 同層序號

機電標單每個配電盤底下都有一模一樣的 `-3 | D-FUSE 500V 2A W/BASE`、
`-10 | PVC WIRE&五金另料&壓克力銘牌`。只比對代號+名稱,「藥局盤的 D-FUSE」
會撞上「K1 盤的 D-FUSE」,後面章節那幾列被判成重複 → **跳過不插入**,
標單短少;被撞到的那列還會被 update 成後面那列的位置與數量。
YM-2026-001 就是這樣:標單解析出 339 列,DB 只有 315 列。

`buildMergeKeys`(`lib/import-plan.ts`,純函式有單元測試)改用
「從 root 到自己的 (項次代號|名稱) 路徑 + 同層同鍵出現序號」:
- 父層自己也可能同名(該案有兩個 `"E1L" PANEL`),所以路徑每一層都帶項次代號。
- 序號處理「檔案裡真的有兩列一模一樣的同層工項」(該案 0027.0001 就有兩份),
  沒有序號第二列會永遠被當成第一列的重複而消失。
- ⚠ DB 端與解析端**必須用同一個母體**建鍵:DB 傳整案的列,解析端傳 usable
  (實際會寫進 DB 的節點)。混入使用者勾略過的節點會讓序號錯位 → 重匯變重複插入。

### 2. 進度算法(`lib/case-progress.ts`,三頁共用)

以前案件列表 / 儀表板 / 案件總覽報表各寫一份,連「要不要算草稿」都不一致。
收斂成 `computeCaseProgress` 一份,同時修掉:

- **percent 記在「數量 0」工項上會被歸零**(真 bug)。舊寫法把百分比乘上契約數量
  換成絕對量、算進度時再除回去;「式」計價工項數量是 0,一乘就變 0 —— 主任填
  「這項做完 100%」完全不算數。YM-2026-001 有 11 筆被吃掉(270 項裡 52 項數量為 0)。
  現在 percent 直接就是完成比例,不做這趟往返。
- **主數字改產值加權**(完成產值 ÷ 合約總價),旁邊小字標工項完成率。
  未加權平均把 NT$223,000 的「配線另料」跟 NT$535 的 D-FUSE 各算 1/270,
  小項做完一堆時嚴重高估:該案未加權 23.8%、產值加權只有 3.4%。
  整案沒有任何單價時(例:大湖山莊)退回工項完成率,不會變成「—」。
- **草稿不算、退回要算**。退回多半是補照片改錯字,混凝土不會因為日誌被退回就沒灌;
  把退回當零產值,一份 92 項的日誌被退回時整案進度會瞬間掉到 0 再跳回來。
- `skipped` 勾略過的工項不列入分母;section / extra / unsigned 一樣不進總進度。

### 3. 「進度落後」紅燈:固定門檻 → 比對工期

舊規則「< 30% 且開工 > 60 天」在產值加權下沒有意義(新案產值本來就接近 0,
會整排變紅)。改成 `expectedProgressPct`(線性:已過工期比例)與實際進度相比,
**落後 20 個百分點以上**才算落後。需要 `cases.started_at` + `cases.expected_end`
兩個日期都有填才作動 —— 算不出來就不判斷,不亂報警。
⚠ 2026-08-04 當下 11 個案件的這兩個欄位**全是空的**,所以紅燈實質上不會亮;
要讓它有用,得先把開工日與預定完工日補進去。

### 4. 一次性資料修補(production)

08-03 之前那次壞掉的匯入 + 撤銷,已經在 YM-2026-001 留下爛資料,用 SQL 修:
- 103 筆日誌引用指向已刪工項 → 96 筆改接到重新匯入後的對應工項,
  另 7 筆現有標單樹真的沒有的,照 audit 快照補回原列(沿用原 id)。
- 28 組重複工項(同章節同位置同名兩份,所有欄位一致)合併後刪除。
- 修改前的原值都先寫進 `audit_logs`(retention 1 年)才動,全程包 transaction
  + 自我檢查,不符預期就整筆 rollback。
- 教訓:`daily_logs.work_items` 用 jsonb 存 work_item_id 而沒有 FK,
  工項被刪時不會擋、也不會 cascade,只能靠事後對帳發現。

## 2026-08-04（續）— 簽核意見發站內消息 + 助理不用等退回就能改（migration-2.33）

業主(Phil)在 LINE 上的兩件回饋,同一批做完:

> 「日誌我核過的,但是我有在下面給意見,可是底下的人他們那裡不會跳通知出來,
>  所以他們也不會知道。」
> 「然後辦公室助理要編輯日誌,都只能等我退回才能編輯,沒有辦法主任呈上來就編輯。」

### 1. 通知從「只有 LINE」變成兩條路(app_messages)

- **不是 bug,是設計缺口**:`sendNotification()` 第二步就把「沒綁 LINE / 沒開
  分類開關」的人整批濾掉。而底下的人**都沒綁**,Phil 也確認過
  「我有問過他們也沒有想要的意思」「助理說用電腦比較習慣」—— 所以不是去推綁定,
  是要一條**不需要綁定**的通道。
- **選站內消息而不是 email / 簡訊**:他們每天本來就會登入這個系統填日誌 / 審核,
  紅點就在他們眼前;email 沒人在收、簡訊要錢。站內消息也不吃官方帳號的
  200 則/月額度,所以可以放心逐份發(批簽也逐份發,不必像 LINE 那樣做彙總)。
- **「有意見再有消息就好」是業主拍板的**(Evelyn 問「有意見再有消息,還是有核過
  就有消息?」→ Phil:「我覺得有意見,再有消息就好」「不然你看工地那麼多,
  其實我自己本身也沒有辦法每天都去核這個東西」)。所以:
  通過 + 有意見 / 退回 / 強制退回 / 撤回核定 → 發;**通過但沒留意見 → 不發**。
  這條規則直接決定了消息中心會不會變成沒人看的雜訊區。
- **收件人不只主任**:老闆在核定關留的意見,前面審過的助理也該知道
  (Phil 說的「底下的人」是複數)。`loadLogStakeholders()` = 主任 ＋
  `log_approvals` 裡所有經手人,排除留言者自己。
- **消息只是信封,意見正本永遠在 `log_approvals`**。所以 UPDATE policy 允許
  使用者動自己的 row(標已讀)不構成竄改風險,retention 也敢設 90 天。
  INSERT 沒有 policy — 只有 service-role 寫得進去,使用者無法偽造消息。
- **降級要安靜**:migration-2.33 沒跑時,查詢與寫入都吞掉(`isMissingTable`),
  未讀數回 0、收件匣回空 — 不能讓 header 鈴鐺把整個 layout 弄掛。
- 同一批也把**意見在畫面上的份量**修掉:簽核歷程裡的意見以前是灰色小字
  (寫了跟沒寫一樣),改成銅金框 +「意見」標籤;日誌詳情頁頂部另外掛未讀
  banner,看過即標已讀(`after()` 寫,不擋畫面)。核定通過的 LINE 卡片
  也把意見帶進去。
- 簽核頁的備註欄文案改成「意見（選填）— 填了會發消息通知主任」:
  這欄以前叫「備註」,填了不會發生任何事,難怪 Phil 覺得沒人看。

### 2. 助理「不用等退回就能改」— 缺的是入口不是權限

- 查下去權限**早就開好了**:RLS `logs_office_write`(migration-2.10)+
  `saveLogAction` 的 post_edit 分支,助理對 `submitted` / `rejected` 的日誌
  本來就能全權修改。這在 2026-08-01 那批就確認過一次。
- 真正的問題:助理整天待在 `/approvals`,而那兩頁**只有「通過 / 退回」兩顆按鈕**。
  沒有入口 = 沒有這個功能 —— 使用者的認知是對的,只是原因不是權限。
- 補三個入口:審核詳情頁標題右側、待簽列表每張卡片下緣、
  (原本就有的)日誌詳情頁右上「編輯」。文案一律講清楚「不用退回、改動會留紀錄」。
- 助理在 `audit` 關改 → 停在 audit(自己關卡內修正);在 `approve` 關改 →
  退回 audit 重審。這是既有規則,沒有改。
- 教訓:**「使用者說做不到」不一定等於權限不足**,先看他站的那一頁有沒有那顆按鈕。
  這批兩件事都是同一個形狀 — 能力早就在系統裡,只是使用者站的位置看不到。

### 追加(同日)— 「改了日誌也要通知」+ migration-2.33 已上 production

- 業主追加要求:除了意見,**日誌被改**也要發消息。事件 `log_edited`,兩條路徑都接:
  - `saveLogAction` 的 post_edit(助理 / 老闆 / 主任的送出後編輯)→ 帶
    `changed_fields` 轉成中文欄位名(`FIELD_LABEL` 從 log-diff 匯出),
    body 直接寫「改了:施工工項、照片」。
  - classic 重送(主任改完退回的日誌按送出核定)→ 這條路徑不寫 revision、
    算不出差異,`changedLabels` 傳 `null`,body 寫「已修正並重新送出」。
- **首次送出不發消息**:每天每份日誌都發一則會把真正要看的意見洗掉,
  而「有新日誌待審核」導覽列的紅字已經涵蓋。只有**重送**才發 —
  「你退回的那份改好了」是有話要說。
- 重送的消息多發給**全部辦公室助理**(`createAppMessages` 因此加了 `roles` 參數),
  收件人跟既有的 `notifyLogResubmitted`(LINE)一致 — 差別只在這條不需要綁定。
- **migration-2.33 已直接在 production 執行**(2026-08-04,Evelyn 授權,
  經 Supabase MCP `apply_migration`;ref `giclppjyuguylbqvjozx`,以 11 案件 /
  34 日誌 / 24 帳號確認是正式站)。驗證:RLS enabled、2 條 policy、4 個索引、0 筆資料;
  security advisor 沒有因這張表新增任何警告。執行狀態記在 MIGRATIONS.md 第 37 列
  與 migration-2.33.sql 檔頭 — **不要再當成「還沒跑」**。

## 2026-08-04(再續)— 核定雙簽做成開關 + 19 份單簽日誌回填為已核定(migration-2.34)

業主:「第二個核定人還沒到位,暫時先把要有第二個核定人核定完才結案的功能關掉
(或許在設定頁做一個 switch),然後幫我把目前已核定的那些都當做已核定完成。」

### 1. 為什麼是開關,不是拿掉

雙簽是 2026-07-20 業主自己拍板的內控機制(兩個人簽 = 一個人不能獨自放行對外文件)。
現在關掉是**人力現況**而不是規則改變 — 第二位核定人到職那天要能一鍵恢復。
所以做成 `app_settings` 的一列 + `/staff` 頁的 switch,不改程式、不用重新部署。

- **不用「把第二個 owner 帳號停掉」來達成**:`requiredApproveSignatures()` 是數 owner
  帳號數,靠帳號數推規則太隱晦 — 「為什麼日誌卡住?」的答案不該是「因為某個帳號存在」。
  設定要能被看見、被說明、被還原。
- 順手修掉一個潛在卡死:原本數 owner 帳號**沒有濾 `is_active`**,離職的核定人被停用後
  仍被算成「還有第二個人可以簽」,日誌會永遠等不到那一簽。改成只算啟用中的。
- **降級方向要對**:設定讀不到(migration 沒跑 / 查詢失敗 / 那列被刪)一律 fallback
  成**雙簽**,也就是這個功能上線前的行為。安全預設是「比較嚴」的那一邊。
- 開關寫入走 service-role(app_settings 沒有 INSERT/UPDATE policy),角色用
  `requireManager()` 擋;誰改的記在 `updated_by`,並由既有 `audit_trigger_fn` 留一筆
  audit_logs(這也是為什麼這張 key-value 表要有 `id uuid` — trigger 用 NEW.id 當 record_id)。

### 2. 19 份日誌的回填

當時 production 卡在核定關的 19 份**全部**都已經有 Phil 的一簽(0 份零簽名、
0 份雙簽),日期 7/15–8/3,PDF 一份都還沒產。單一 transaction 內:
原值(整列 jsonb)寫進 `audit_logs` → update 成 `approved` + `current_stage=null`
→ 三道自我檢查(筆數必須是 19、不可有零簽名的被標成已核定、更新後狀態必須正確),
任一不符就整筆 rollback。結果:已核定 13 → 32,核定關待辦 0。

- `approve_signatures` 保持 1 **不動** — 那是事實,不要為了好看去改。
  PDF 的簽核欄本來就是照 `log_approvals` 實際筆數畫,一位就畫一格,不會破版。
- **刻意不發通知**:回填是資料修補,不是誰簽了名。19 份 ×(主任+經手人)的消息與 LINE
  只會洗版,而且日誌狀態在他們清單裡本來就看得到。
- 這 19 份的 PDF 要另外補 → 順手把 `/reports/regen-pdfs` 加上「只補還沒有 PDF 的」模式
  (原本只能整批重產,32 份要等很久,其中 13 份根本不缺)。

### 3. 影響與風險(已跟業主說明)

- 單簽期間任何一位核定人都能獨自完成核定 — **包括 `測試-owner` 這種測試帳號**。
  測試完應該把它停用(停用後啟用中的 owner 只剩一位,即使之後把雙簽打開也會自動退回單簽)。
- 這 19 份日誌的 PDF 只會有一位核定人的簽名。文件本身沒有不實 —— 它忠實呈現
...
## 2026-09-08 — 「本日無施工」一鍵送單

業主：沒施工的日子（下雨、放假、等材料）主任也要送日誌，而且**一樣走完整簽核**。
現況是主任手 key「無施工」硬送 — 但表單本來就擋「至少 1 個工項」，實際上送不出去，
production 查了 135 份日誌也找不到固定寫法（零工項的兩份都是助理事後把工項刪掉的）。

### 做法

- **旗標放 `daily_logs.manpower` jsonb**（`no_work: true` + `no_work_reason?: string`），
  沒有 migration。manpower 本來就是「加欄位不用 migration」的擴充點，而「無施工」語意上
  也就是出工 0 人。要篩選時 `manpower->>'no_work'` 一樣查得到。
- **表單**（`/logs/new`、編輯頁共用）：日期／天氣區塊下方多一張「今天沒有施工？」卡，
  按「本日無施工」後 出工 / 一 / 二 / 三 / 四 / 舊資料 整段收起，只剩照片、重要事項、簽名。
  原因是選填（天候不佳 / 例假日 / 等材料 / 業主要求暫停 chips，也可自己打）。
  已勾的工項留在 state（按「取消，今天有施工」會還原），但送出時工項 / 人數 / 外包一律送空。
  localStorage 草稿一併記住旗標。
- **Server**（`saveLogAction`）：`no_work` 時放行零工項；反過來「標了無施工卻還帶工項」
  直接擋（矛盾資料）。其餘流程零改動 — status / stage / 簽名 / 通知 / PDF 觸發都跟平常一樣，
  這正是業主要的「一樣要簽核」。
- **顯示**：列表與待簽核卡片掛「無施工」標籤、詳情頁 / 簽核頁把「未填工項」的空狀態改寫成
  「本日無施工（原因）」、簽核摘要卡的工項數顯示「無施工」、PDF 表頭下多一段「本日無施工」。
  簽核頁的「本案還有 N 個合約內工項本日未填」對照在無施工時**跳過**（不然會把整案列成漏填）。
- **編輯軌跡**（`lib/log-diff.ts`）：助理把無施工改成有工項（或反過來）會列「本日無施工 是→否」
  與原因變化。

### 沒做 / 注意

- 沒有擋「同一天同一案已經有一份有施工的日誌，又送一份無施工」— 跟現況一致（本來就允許
  一天多份，表報編號用 seq 區分），由助理審核時判斷。
- 儀表板「超過 5 天沒新日誌」的紅燈把無施工日誌當成正常日誌計入 — 這是刻意的，
  業主要的就是「沒施工也有紀錄」。
- 待簽核的「無工項」快速篩選仍會把無施工日誌列進去（它們確實零工項），卡片上有標籤可分辨。

## 2026-09-09 — 雙簽拿掉，換成可選的「審閱關」+ 新角色「審閱人」

業主改口兩件事：
1. 核定**維持一位**核定人簽完就產 PDF（第二位核定人始終沒到職，雙簽從 2026-08-04 起就一直關著）。
2. 多一個角色，「在系統上簽核就好，不用進到 PDF 裡」— 性質像顧問，但畫面上不要寫顧問，
   要一個「像是審核什麼的」名字，且不能跟現有關卡撞名。

### 命名

現有關卡：填表（fill）／審核（audit，助理）／核定（approve，核定人）。新的叫**「審閱」**，
角色叫**「審閱人」**（enum `reviewer`）。「審閱」跟「審核」「核定」都不重覆，語意是
「看過、簽個名」，剛好對上「在系統上簽核就好」。曾考慮「會簽」，但尾字是「簽」，跟 PDF 上的
「簽核」容易混。

### 做法

- **stage 沿用 `review`**：這個 enum 值原本是「主任複核」，production 從沒啟用
  （log_approvals 零筆 stage='review'；2026-09-09 查證），直接改給審閱人用，stage enum 不用動。
  role enum 加 `reviewer`（migration-2.35，**要單獨先跑**：新 enum 值不能在同一個
  transaction 裡被 policy 引用）。RLS 與設定在 migration-2.36。
- **位置**：audit 通過後 → review → approve。是**閘門**（審閱人不簽日誌不會往前走），
  否則「簽核」沒意義。
- **可開關 + 自動跳過**：`app_settings.approval.review_stage_enabled`（預設 false）；
  開著但沒有任何啟用中的審閱人 → 自動跳過（`isReviewStageActive()`）。這是雙簽時代
  「只有一位老闆就卡死」的教訓直接搬過來。人員管理頁原本的「核定簽名規則」開關換成「審閱關」。
- **不進 PDF**：`omitReviewStage()` 在 PDF 元件入口就把 review 關的簽名**與意見**都濾掉。
  意見也濾是為了一致 — PDF 對外，審閱人的痕跡一律不出現；系統內的簽核歷程、「我簽過的」照常。
- **雙簽整套刪除**：`lib/approvals/dual-sign.ts`、`approve_signatures` 的讀寫、
  「已簽過的先不顯示」過濾、「通知另一位核定人」、批簽的 awaitingSecond 全部拿掉；
  核定關變成一個條件式 UPDATE（status=approved）。欄位 `approve_signatures` 留在 DB 不刪。
- **role↔stage 對照集中到 `lib/approvals/stages.ts`**：以前 actions / approvals 頁 /
  詳情頁 / layout / pending-signables 各抄一份，這次一併收掉。
- **主任的「待複核」入口拿掉**：那一頁永遠是空的（複核關從沒啟用），手機 tab 換成「請假」
  （主任要簽現場人員的假單，以前手機版沒入口）。
- 審閱人能看的：待審閱、我簽過的、日誌（唯讀）、案件總覽（唯讀）、請假（自己的，直接給
  owner 簽）、消息。不給儀表板／人員／報表。請假階層不把 reviewer 插進去 — 他不簽別人的假。
- LINE：新事件 `log_to_review` / `log_batch_to_review`（分類同「日誌待簽核」）；
  審閱人沒有自己的 Rich Menu 圖，借用助理版。

### 上線順序

程式先 push（沒有 reviewer 帳號、設定不存在 → 行為 = 三關單簽，跟現況一樣），再跑
