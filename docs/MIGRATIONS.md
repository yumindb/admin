...
| 41 | [`migration-2.37.sql`](migration-2.37.sql) | 審閱改成「加簽」(跟流程無關):拿掉 2.36 的 `logs_reviewer_stage_update` policy 與 `approval.review_stage_enabled` 設定。保留 reviewer 寫 review 關紀錄的 policy 與 profiles 讀取 | 必跑 |
